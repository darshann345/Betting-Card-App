...
router.post("/", auth, async (req, res) => {
  try {
    ...
    const game = new Game({
      user: req.user._id,
      betAmount,
      cards: [...generateRow(nums, "top"), ...generateRow(nums, "bottom")],
      status: "playing",
      matchedPairs: 0,
      attempts: 0,
    });
    ...
    if (game.matchedPairs === 5) {
      ...
      if (admin) {
        if (admin.balance < game.winnings) {
          return res
            .status(400)
            .json({ error: "Admin has insufficient balance" });
        }
      ...
      req.user.totalWinnings = (req.user.totalWinnings || 0) + game.winnings;
      req.user.gamesWon = (req.user.gamesWon || 0) + 1;
      await req.user.save();
...
router.get("/current", auth, async (req, res) => {
  const game = await Game.findOne({ user: req.user._id }).sort({
    createdAt: -1,
  });
